...
gplots::plotCI(add=T, x=clientsST, y=meanST, xlim=xrange, uiw=sdST, type="o", pch=20, gap=0, ylim=yrangeMean, col="red", xlab="", ylab="", xaxt="n", sfrac=0.005)
...
text(par("usr")[1]-25, yMarksCulled, adj=1, labels=yMarksOrigCulled, xpd=T, cex=0.6)
...
legend(list(x=10,y=250), legend=c("SE", "ST"), col=c("blue", "red"), lty = 1, pch=20, bty="n", cex=0.7)
